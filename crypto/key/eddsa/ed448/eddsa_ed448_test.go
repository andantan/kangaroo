...
func Test_EdDSA_Ed448_PrivateKey_Lifecycle(t *testing.T) {
	// 1. Generation
	privKey, err := GenerateEdDSAEd448PrivateKey()
	require.NoError(t, err)
	assert.True(t, privKey.IsValid())
	assert.Equal(t, eddsa.EdDSAEd448Type, privKey.Type())

	// 2. Bytes Round Trip
	privKeyBytes := privKey.Bytes()
	assert.Equal(t, eddsa.EdDSAEd448PrivateKeyBytesLength, len(privKeyBytes))
	reloadedPrivKey, err := EdDSAEd448PrivateKeyFromBytes(privKeyBytes)
	require.NoError(t, err)
	assert.Equal(t, privKey, reloadedPrivKey)
}

func Test_EdDSA_Ed448_PublicKey_Lifecycle(t *testing.T) {
	...
	for _, tc := range addressSuites {
		t.Run(fmt.Sprintf("with %s address deriver", tc.Name), func(t *testing.T) {
			...
			assert.Equal(t, eddsa.EdDSAEd448Type, pubKey.Type())

			// 2. Bytes Round Trip
			pubKeyBytes := pubKey.Bytes()
			assert.Equal(t, eddsa.EdDSAEd448PublicKeyBytesLength, len(pubKeyBytes))
			reloadedPubKey, err := EdDSAEd448PublicKeyFromBytes(pubKeyBytes)
			require.NoError(t, err)
...
func Test_EdDSA_Ed448_Signature_Lifecycle(t *testing.T) {
	...
	for _, tc := range hashSuites {
		t.Run(fmt.Sprintf("with %s hash", tc.Name), func(t *testing.T) {
			...
			assert.Equal(t, eddsa.EdDSAEd448Type, signature.Type())

			sigBytes := signature.Bytes()
			assert.Equal(t, eddsa.EdDSAEd448SignatureBytesLength, len(sigBytes))
			reloadedSig, err := EdDSAEd448SignatureFromBytes(sigBytes)
			require.NoError(t, err)
...
func Test_EdDSA_Ed448_Signature_Verify(t *testing.T) {
	hashSuites := testutil.GetHashSuiteTestCases(t)
	...
	for _, tc := range hashSuites {
		t.Run(fmt.Sprintf("with %s hash", tc.Name), func(t *testing.T) {
			...
			t.Run("Verification with wrong key should fail", func(t *testing.T) {
				otherPrivKey, err := GenerateEdDSAEd448PrivateKey()
			...
			t.Run("Verification with invalid public key should fail", func(t *testing.T) {
				invalidPubKeyBytes := make([]byte, eddsa.EdDSAEd448PublicKeyBytesLength)
				invalidPubKey, err := EdDSAEd448PublicKeyFromBytes(invalidPubKeyBytes)
				assert.NoError(t, err)
...
func Test_EdDSA_Ed448_Key_Wrapper_RoundTrip(t *testing.T) {
	hashSuites := testutil.GetHashSuiteTestCases(t)
