...
func Test_ECDSA_Secp256r1_PrivateKey_Lifecycle(t *testing.T) {
	...
	assert.Equal(t, ecdsa.ECDSASecp256r1PrivateKeyBytesLength, len(privKeyBytes))
...
func Test_ECDSA_Secp256r1_PublicKey_Lifecycle(t *testing.T) {
	...
	for _, tc := range addressSuites {
		t.Run(fmt.Sprintf("with %s address deriver", tc.Name), func(t *testing.T) {
			...
			assert.Equal(t, ecdsa.ECDSASecp256r1PublicKeyBytesLength, len(pubKeyBytes))
...
func Test_ECDSA_Secp256r1_Signature_Lifecycle(t *testing.T) {
	...
	for _, tc := range hashSuites {
		t.Run(fmt.Sprintf("with %s hash", tc.Name), func(t *testing.T) {
			...
			assert.Equal(t, ecdsa.ECDSASecp256r1SignatureBytesLength, len(sigBytes))
	...
	for _, tc := range hashSuites {
		t.Run(fmt.Sprintf("with %s hash", tc.Name), func(t *testing.T) {
			...
			t.Run("Verification with invalid public key should fail", func(t *testing.T) {
				invalidPubKeyBytes := make([]byte, ecdsa.ECDSASecp256r1PublicKeyBytesLength)
...
func Test_ECDSA_Secp256r1_Key_Wrapper_RoundTrip(t *testing.T) {
	hashSuites := testutil.GetHashSuiteTestCases(t)
